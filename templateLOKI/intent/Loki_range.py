...
MODULE_DICT = {
    "Account": import_from_path("templatePOC_lib_Account", os.path.join(os.path.dirname(CWD_PATH), "lib/Account.py")),
    "LLM": import_from_path("templatePOC_lib_LLM", os.path.join(os.path.dirname(CWD_PATH), "lib/LLM.py"))
}
"""
Account 變數清單
[變數] BASE_PATH         => 根目錄位置
...
def getResult(inputSTR, utterance, args, resultDICT, refDICT, pattern="", toolkitDICT={}):
    ...
    if utterance == "交易區間為[30.71元]至[30.735元]":
        if CHATBOT:
            ...
            resultDICT["range"] = (args[0].strip("元"), args[1].strip("元"))

    if utterance == "最高[30.579元]、最低[30.65元]":
        if CHATBOT:
            replySTR = getReply(utterance, args)
            if replySTR:
                resultDICT["response"] = replySTR
...
if __name__ == "__main__":
    ...
    resultDICT = getResult("交易區間為30.71元至30.735元", "交易區間為[30.71元]至[30.735元]", [], {}, {})
    pprint(resultDICT)