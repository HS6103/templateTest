...
MODULE_DICT = {
    "Account": import_from_path("templatePOC_lib_Account", os.path.join(CWD_PATH, "lib/Account.py")),
    "LLM": import_from_path("templatePOC_lib_LLM", os.path.join(CWD_PATH, "lib/LLM.py")),
    "Project": import_from_path("templatePOC_lib_Project", os.path.join(CWD_PATH, "lib/Project.py")),
}
"""
Account 變數清單
[變數] BASE_PATH         => 根目錄位置
...
if __name__ == "__main__":
    ...
    if resultDICT:
        ...
        templateSTR = f"The U.S. dollar {resultDICT["usd_up_down"][0]} against the Taiwan dollar {weekday}, {tmpSTR} NT${resultDICT["up_down_num"][0]} to close at NT${resultDICT["close"][0]}.\n\nTurnover totaled US${resultDICT["turnover"][0]} during the trading session.\n\nThe greenback opened at NT${resultDICT["open"][0]}, and moved between NT${resultDICT["range"][0][0]} and NT${resultDICT["range"][0][1]} before the close."
